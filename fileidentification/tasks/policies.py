import json
from collections.abc import Callable, Iterable
from dataclasses import dataclass
from pathlib import Path

from fileidentification.definitions.models import (
    LogMsg,
    Mode,
    Policies,
    PoliciesFile,
    PolicyParams,
    RunJournal,
    SfInfo,
)
from fileidentification.definitions.settings import DEFAULTPOLICIES, FMT_INFO, FDMsg, PLMsg
from fileidentification.tasks.os_tasks import remove
from fileidentification.workspace import Workspace
from fileidentification.wrappers.ffmpeg import ffmpeg_media_info


def build_policies(
    puids: Iterable[str],
    default_policies: Policies,
    mode: Mode,
    *,
    blank: bool = False,
    extend: bool = False,
    existing: Policies | None = None,
) -> tuple[Policies, list[str]]:
    """
    Build the policy map for the encountered puids. Pure: no file I/O, no shared-state mutation.
    Returns (policies, blank_puids) where blank_puids are the puids that received an empty fallback policy.
    """
    policies: Policies = {}
    blank_puids: list[str] = []

    if blank:
        for puid in puids:
            policies[puid] = PolicyParams(format_name=FMT_INFO[puid].name, remove_original=mode.REMOVEORIGINAL)
        return policies, blank_puids

    for puid in puids:
        if puid in default_policies:
            policies[puid] = default_policies[puid]
        # no default for this filetype and not strict: add an empty (blank) policy
        if not mode.STRICT and puid not in default_policies:
            policies[puid] = PolicyParams(format_name=FMT_INFO[puid].name)
            blank_puids.append(puid)
        # extend: keep an already existing policy for this puid
        if extend and existing and puid in existing:
            policies[puid] = existing[puid]
            if puid in blank_puids:
                blank_puids.remove(puid)
        # propagate remove_original mode
        if puid in policies and mode.REMOVEORIGINAL:
            policies[puid].remove_original = True

    return policies, blank_puids


class PolicyError(Exception):
    """An external policies.json is missing or invalid. The caller persists run state and exits."""


@dataclass
class PolicyResolution:
    """
    Outcome of resolving policies for a run: the policy map, the blank-fallback PUIDs (None if not generated),
    and the file-level sipi_guard flag (DaSCH: verify images with sipi during inspection).
    """

    policies: Policies
    blank: list[str] | None = None
    sipi_guard: bool = False


def _read_policies_file(policies_path: Path) -> PoliciesFile:
    """Read and validate a policies.json into its PoliciesFile model. Raises PolicyError if missing / invalid."""
    if not policies_path.is_file():
        raise PolicyError(f"{policies_path} not found")  # noqa: EM102, TRY003
    try:
        return PoliciesFile(**json.loads(policies_path.read_text()))
    except ValueError as e:
        raise PolicyError(str(e)) from e


def read_policies(policies_path: Path) -> Policies:
    """Read and validate a policies.json, returning its PUID->policy map."""
    return _read_policies_file(policies_path).policies


def _write_policies(
    outpath: Path, policies: Policies, *, blank: bool, extend: bool, strict: bool, sipi_guard: bool = False
) -> None:
    """Serialize a generated / extended policies map to outpath with a provenance comment."""
    comment = "autogenerated"
    if blank:
        comment += " blank policies"
    else:
        if extend:
            comment += f" updating from {outpath}"
        comment += f" using default policies {DEFAULTPOLICIES}"
        if strict:
            comment += " in strict mode"
    jsonfile = PoliciesFile(name=outpath, comment=comment, sipi_guard=sipi_guard, policies=policies)
    jsonfile.name.write_text(jsonfile.model_dump_json(indent=4, exclude_none=True))


def resolve_policies(
    puids: Iterable[str],
    poljson: Path,
    mode: Mode,
    *,
    policies_path: Path | None = None,
    blank: bool = False,
    extend: bool = False,
    emit: Callable[[str], None] = lambda _msg: None,
) -> PolicyResolution:
    """
    Resolve the policy map for a run and return it (no shared-state mutation).

    either the default location (_policies.json) if present, an external file passed via policies_path
    or a freshly generated map from the default policies (optionally blank).
    extend merges the filetypes found in the folder onto the resolved map if not already present.
    A generated or extended map is (re)written to default location; only reading an existing map does not write.
    """
    # a policies.json at the default location and no external one passed -> read from there
    if not policies_path and poljson.is_file():
        policies_path = poljson

    # nothing to read (or blank requested) -> generate from the default policies (carry its sipi_guard through)
    if not policies_path or blank:
        emit("Generating policies")
        default = None if blank else _read_policies_file(DEFAULTPOLICIES)
        default_policies: Policies = default.policies if default else {}
        sipi_guard = default.sipi_guard if default else False
        policies, blank_puids = build_policies(puids, default_policies, mode, blank=blank)
        _write_policies(poljson, policies, blank=blank, extend=False, strict=mode.STRICT, sipi_guard=sipi_guard)
        resolution = PolicyResolution(policies, None if blank else blank_puids, sipi_guard=sipi_guard)
    # read the passed / default-location policies as-is
    else:
        emit(f"Loading policies from {policies_path}")
        pf = _read_policies_file(policies_path)
        resolution = PolicyResolution(pf.policies, sipi_guard=pf.sipi_guard)

    # extend: add filetypes the resolved map is missing, keep the existing (hand-tuned) entries
    if extend:
        emit(f"Updating the filetypes in policies {poljson}")
        policies, blank_puids = build_policies(
            puids, read_policies(DEFAULTPOLICIES), mode, extend=True, existing=resolution.policies
        )
        _write_policies(
            poljson, policies, blank=False, extend=True, strict=mode.STRICT, sipi_guard=resolution.sipi_guard
        )
        resolution = PolicyResolution(policies, blank_puids, sipi_guard=resolution.sipi_guard)

    return resolution


def apply_policy(sfinfo: SfInfo, policies: Policies, ws: Workspace, journal: RunJournal, strict: bool) -> None:
    """
    Flag files for conversion ; in strict mode move an unlisted format to _REMOVED (else skip with a log).
    Accepted A/V (fmt/199, fmt/569) is stream-checked.
    """
    sfinfo.status.applied = True  # mark it so a re-run does not re-evaluate / re-log it
    puid = sfinfo.processed_as
    if not puid:
        return
    if sfinfo.status.pending:
        return

    if puid not in policies:
        # in strict mode, move file
        if strict:
            sfinfo.processing_logs.append(LogMsg(name="fidr", msg=f"{PLMsg.NOTINPOLICIES}"))
            remove(sfinfo, ws, journal)
            return
        # just flag it as skipped
        sfinfo.processing_logs.append(LogMsg(name="fidr", msg=f"{PLMsg.SKIPPED}"))
        return

    # case where file needs to be converted
    if not policies[puid].accepted:
        sfinfo.status.pending = True
        return

    # check if mp4 / mkv has correct stream (i.e. h264 and aac)
    if puid in ["fmt/199", "fmt/569"] and _has_invalid_streams(sfinfo, puid, ws, journal):
        sfinfo.status.pending = True
        return


def _has_invalid_streams(sfinfo: SfInfo, puid: str, ws: Workspace, journal: RunJournal) -> bool:
    """Return true if video and audio codec differ from archival standards"""
    streams = ffmpeg_media_info(ws.abs_path(sfinfo.filename))
    if not streams:
        # ffprobe read no streams for a file we meant to stream-check: record a warning for the end-of-phase report
        journal.diagnose(
            sfinfo, FDMsg.WARNING, LogMsg(name="ffmpeg", msg="could not read streams for the a/v stream check")
        )
        return False
    if puid in ["fmt/569"]:
        # only the video codec has to be ffv1 -> return false as soon as any stream is ffv1
        return all(stream["codec_name"] not in ["ffv1"] for stream in streams)
    if puid in ["fmt/199"]:
        # video codec has to be h264, audio codec aac -> return true if any a/v stream does not match
        for stream in streams:
            if stream["codec_type"] not in ["video", "audio"]:
                continue
            if stream["codec_name"] not in ["h264", "aac"]:
                return True
    return False
